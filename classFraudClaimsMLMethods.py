...
#======================================================================================================================================
# Class definition
class fraudClaimsMLMethodsPipeline:
    # Global variables.
    global lstModelCode
    global dictModelHyperParams
    global dictCalibCVParams
    global GetTrainedLogRegressionModel
    global GetLogRegressionModelHyperParameters
    # List to store Logistic Regression config types.
    lstModelCode         = ['LgR_1001','LgR_1002']
    # Hash table to store CalibratedCV parameters.
    dictCalibCVParams = {"method":"sigmoid","cv":"3"}
    # Nested hash table to store model hyper-parameters.
    dictModelHyperParams = {"LgR_1001" :{
                                                "classWeight":"1",
                                                "penalty":"l1",
                                                "cv":"5",
                                                "solver":"liblinear",
                                                "max_iter":"1000",
                                                "alpha":{'C': [0.0001,0.001, 0.01, 0.1, 1, 10, 100,1000,10000]}                     
                                         },
                             "LgR_1002" :{
                                                "classWeight":"1",
                                                "penalty":"l2",
    ...
    def standardize_data(self,
        ...
        if not(SkipNormalize):
            ...
            if IsNormalize == 1:
                ...
                # Instantiate the MiniMax Scaler
...
#======================================================================================================================================
    ...
    def GetTrainedDTreeModel(self,
        ...
        return dt_optimal    
#======================================================================================================================================    
    def GetLogRegressionModelHyperParameters(self,
                                             X_train_stdzd,
                                             Y_train_ravel,
                                             modelCode
                                            ):
        dictHyperParams = {}
        dictHyperParams = dictModelHyperParams.get(modelCode)
        penalty         = dictHyperParams.get('penalty')
        classWeight     = int(dictHyperParams.get('classWeight'))
        cv              = int(dictHyperParams.get('cv'))
        solver          = dictHyperParams.get('solver')
        max_iter        = int(dictHyperParams.get('max_iter'))
        param_grid      = dictHyperParams.get('alpha')
...
#======================================================================================================================================    
    # Method to get the optimal Hyper-parameter.
    def  GetModelHyperParameters(self,
                                 X_train_stdzd,
                                 Y_train_ravel,
                                 modelCode
                               ):
        if (modelCode in lstModelCode):
            gridResults,bestScore,optimal_HyperParameter = GetLogRegressionModelHyperParameters(self, # use this when called within class.
                                                                                                 X_train_stdzd,
                                                                                                 Y_train_ravel,
                                                                                                 modelCode)
        else:
            print('Hyper-parameters for Model configuration "{}" not found !!!'.format(modelCode))
...
#======================================================================================================================================    
    def GetTrainedModel(self,
                       ...
                       modelCode):
        if (modelCode in lstModelCode):
            lr_optimal = GetTrainedLogRegressionModel(self,
                                                     X_train_stdzd,
                                                     Y_train_ravel,                        
                                                     dictOptimalHyperParameter,
                                                     modelCode)
            ...
            print('Trained Model for "{}" not found !!!'.format(modelCode)) 
            # Return None
            return None                
#======================================================================================================================================
    # Method to return the non-calibrated model after training.
    def GetTrainedLogRegressionModel(self,
                                     X_train_stdzd,
                                     Y_train_ravel,                        
                                     dictOptimalHyperParameter,
                                     modelCode):
        # Get the hyper-parameter lambda from the dictionary.
        hpLambda = float(dictOptimalHyperParameter.get('lambda'))
        
        # Get the other parameters of the Logistic Regression model.
        dictHyperParams = {}
        dictHyperParams = dictModelHyperParams.get(modelCode)
        penalty         = dictHyperParams.get('penalty')
        classWeight     = int(dictHyperParams.get('classWeight'))
        ...
        if classWeight != 0:
            ...
            lr_optimal = LogisticRegression(penalty=penalty,           # use L2 regularizer.
                                            ...
                                            C=(hpLambda),              # use Inverse of Lambda.
        ...
        lr_optimal.fit(X_train_stdzd,Y_train_ravel)
...
#======================================================================================================================================     
    ...
    def GetCalibratedModel(self,
        ...
        # Set CalibratedCV parameters.
        method = dictCalibCVParams.get('method')
        cv     = int(dictCalibCVParams.get('cv'))
        # Instantiate the CalibratedClassifierCV model.
        calibratedCCV = CalibratedClassifierCV(lr_optimal,    # Base model.
                                               method=method, # sigmoid function.
                                               cv=cv)         # crossvalidation #'s.
